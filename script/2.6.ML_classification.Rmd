...
to_label_pengu <- read_rds("data/Adélie_data_18-19_EM/unknown_unbalanced_for_prediction.rds")
my_pred <- predict(best_model, to_label_pengu)
to_label_pengu$predicted_state <- my_pred$.pred_class


to_label_pengu$ml_success <- ifelse(to_label_pengu$predicted_state == to_label_pengu$state & to_label_pengu$predicted_state == "Hunting", "True Hunt",
                                    ifelse(to_label_pengu$predicted_state == to_label_pengu$state & to_label_pengu$predicted_state == "Other", "True Other",
                                           ifelse(to_label_pengu$predicted_state != to_label_pengu$state & to_label_pengu$predicted_state == "Hunting", "Hunt that should be other",
                                                  ifelse(to_label_pengu$predicted_state != to_label_pengu$state & to_label_pengu$predicted_state == "Other", "Other that should be hunt", NA))))

table(to_label_pengu$ml_success)
...
ml_hunt <- to_label_pengu %>%
  group_by(groupid, predicted_state, trip_id) %>%
  summarise(duration = n(), start = min(timestamp), end = max(timestamp))
ref_hunt <- to_label_pengu %>%
  group_by(groupid_ref, state, trip_id) %>% 
  summarise(duration = n(), start = min(timestamp), end = max(timestamp))

#Compare number of hunting chunks
chunk_tibble <- left_join(ml_hunt %>% filter(predicted_state == "Hunting") %>% group_by(trip_id) %>% summarise(n_chunk_pred = n()),
          ref_hunt %>% filter(state == "Hunting") %>% group_by(trip_id) %>% summarise(n_chunk = n()))

ggplot(data = chunk_tibble, aes(x = n_chunk, y = n_chunk_pred)) +
  geom_point() + 
  geom_abline(slope = 1) +
  coord_equal(ratio = 1, ylim = c(0,4000), xlim = c(0,4000))

#Compare time spent hunting
time_tot_tibble <- left_join(ml_hunt %>% filter(predicted_state == "Hunting") %>% group_by(trip_id) %>% summarise(tot_time_pred_hunt_hrs = sum(duration)/60/60),
          ref_hunt %>% filter(state == "Hunting")  %>% group_by(trip_id) %>% summarise(tot_time_hunt_hrs = sum(duration)/60/60))

time_tot_tibble <- left_join(time_tot_tibble, ref_hunt %>% group_by(trip_id) %>% summarise(tot_time_hrs = sum(duration)/60/60))

#plot time spent hunting
duration_plot <- ggplot(data = time_tot_tibble, aes(x = tot_time_pred_hunt_hrs, y = tot_time_hunt_hrs)) +
  geom_smooth(method = "lm", col = "black", alpha = 0.3) +
  geom_point(aes(col = tot_time_hrs)) +
  geom_abline(slope = 1) +
  geom_text(aes(x = 2, y = 4, label = paste("ρ = ", round(cor(tot_time_hunt_hrs, tot_time_pred_hunt_hrs), 2)))) +
  coord_equal(ratio = 1, xlim = c(1,5), ylim = c(1,5)) +
  scale_color_viridis(discrete = "F") + 
  labs(x = "TDR-estimated time spent hunting (hours)", y = "Time spent hunting from accelerometry (hours)", col = "Trip duration (hours)")
duration_plot
```

We detect less chunks because resolution is lower, but overall the time spent hunting is really well predicted ! (R²= 0.77)

Looking at one individual dive also allow us to see how well the RF
worked.

```{r plot dive}
sub_pengu <- to_label_pengu %>% filter(trip_id == to_label_pengu$trip_id[1])

rf_plot <- ggplot(sub_pengu, aes(x = timestamp, y = -depth, col = predicted_state)) +
  geom_point(size = 2) +
  labs(title = "A. (PRED)", x = "", y = "Depth", col = "Behaviour") +
  coord_cartesian(xlim = c(sub_pengu$timestamp[12740], sub_pengu$timestamp[13080])) +
  scale_color_viridis(discrete = T, direction = -1) +
  labs_pubr()
rf_plot
ref_plot <- ggplot(sub_pengu, aes(x = timestamp, y = -depth, col = state)) +
  geom_point(size = 2) +
  labs(title = "B. (REF)", y = "Depth", col = "Behaviour", x ="Timestamp") +
  coord_cartesian(xlim = c(sub_pengu$timestamp[12740], sub_pengu$timestamp[13080])) +
  scale_color_viridis(discrete = T, direction = -1) +
  labs_pubr()

svg("figure/hunt_rf/rf_prediction_dive_lvl_grid.svg", height = 7, width = 10)
cowplot::plot_grid(rf_plot, ref_plot, ncol = 1) 
dev.off()
```

Then, looking at the duration of each of these events, we also see that
the distribution is very similar to the reference.

```{r plot duration}
ref_dur_hist <- ggplot(ref_hunt %>% filter(state == "Hunting"), aes(x = duration)) +
  geom_histogram(binwidth = 1) +
  coord_cartesian(xlim = c(0,60), ylim = c(0,16000)) +
  labs(title = "A.",subtitle = "Reference label", x = "Duration", y = "Count") +
  labs_pubr()

ml_dur_hist <- ggplot(ml_hunt %>% filter(predicted_state == "Hunting"), aes(x = duration)) +
  geom_histogram(binwidth = 1) +
  coord_cartesian(xlim = c(0,60), ylim = c(0,16000)) +
  labs(title = "B.",subtitle = "TDR-based prediction", x = "Duration", y = "Count") +
  labs_pubr()

svg("figure/hunt_rf/hist_hunt_duration_rf_prediction.svg", height = 5, width = 10)
cowplot::plot_grid(ref_dur_hist, ml_dur_hist, ncol = 2)
dev.off()
```
