...
dlw <- dlw[,c(1:3, 16, 31)]
colnames(dlw) <- c("id", "bm_init", "bm_fin", "energy_exp_raw", "comment")
...
for(my_axy in list_axy){
  ...
  #ggplot(dive_label, aes(x = timestamp_max_depth, y = max_depth, col = type, group = 1)) + geom_point()
  my_tdr <- left_join(my_tdr, dive_label, by = "bout_id")
  
  # ggplot(my_tdr, aes(x = timestamp, y = depth, col = type)) + geom_point() + coord_cartesian(ylim = c(0,5))
...
dlw$dee <- dlw$energy_exp_raw/dlw$mass
